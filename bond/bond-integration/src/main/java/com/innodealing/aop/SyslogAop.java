...
public class SyslogAop {
	...
	@Pointcut("execution(* com.innodealing.service..*.*(..)) && ! @annotation(com.innodealing.aop.NoLogging)") // the pointcut expression
	...
	public Object doAfter(ProceedingJoinPoint jp) throws Throwable {
		...
		String methodInfo = getJoinPointInfo(jp) + 
				", ret:" + ((ret != null)? ret : "") +
	...
	public void doAfterThrow(JoinPoint jp,Exception e) {
		LoggerFactory.getLogger(jp.getTarget().getClass()).
			error("Exception, " + getJoinPointInfo(jp), e);
	}
	...
	static private String getJoinPointInfo(JoinPoint point) 
	{
		...
		return "method:" + point.getSignature().getName() + 
