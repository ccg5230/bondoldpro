...
public class DateConvertUtil {
	...
	public static BigDecimal formatterExpressionByTaobao(String expression, OriginalData od, Map<String,String> expressions, String finDate)
		...
		try {
            result = calculate(expression, data, calculate);
        } catch(Exception e) {
            logger.error("calculate expression=" +expression +"  is error",e );
//            e.printStackTrace();
            throw e;//财报计算要抓取异常记录日志
        }
		return result;
	}
	...
	private static BigDecimal doPP(OriginalData od, String expression, Calculate calculate, String finDate) 
		...
		//上年年报财报数据
        String pyFinDate = DateUtils.ppAnnual(finDate, 1);
        Map<String,Object> pyDatas = od.getIndicatorItems().get(pyFinDate);
        //上2年年报财报数据
        String py2FinDate = DateUtils.ppAnnual(finDate, 2);
        Map<String,Object> py2Datas = od.getIndicatorItems().get(py2FinDate);
        //上3年年报财报数据
        String py3FinDate = DateUtils.ppAnnual(finDate, 3);
        Map<String,Object> py3Datas = od.getIndicatorItems().get(py3FinDate);
		BigDecimal result = null;
		//PP函数处理结果null要替换为0
		if(expression.contains("pp3(")){
			...
			result = result==null? new BigDecimal(0) : result;
		}else if(expression.contains("py(")){
            expression = expression.replaceAll("py", "");
            result = calculate.calculate(pyDatas, expression);
            result = result==null? new BigDecimal(0) : result;
        }else if(expression.contains("py2(")){
            expression = expression.replaceAll("py2", "");
            result = calculate.calculate(py2Datas, expression);
            result = result==null? new BigDecimal(0) : result;
        }else if(expression.contains("py3(")){
            expression = expression.replaceAll("py3", "");
            result = calculate.calculate(py3Datas, expression);
            result = result==null? new BigDecimal(0) : result;
        }else{
		...
		if(pyDatas != null){
            currentDatas.put("py", 1);
        }else{
            currentDatas.put("py", 0);
        }
        if(pyDatas != null && py2Datas != null){
            currentDatas.put("py2", 1);
        }else{
            currentDatas.put("py2", 0);
        }
        
        if(pyDatas != null && py2Datas != null && py3Datas!= null){
            currentDatas.put("py3", 1);
        }else{
            currentDatas.put("py3", 0);
        }
		return result;
	}
