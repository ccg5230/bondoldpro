...
@Component
public class DataSourceInsu extends DataSource {
	...
	public Map<String, String> getExpressions() {
		//获取所有表达式
		String sqlQueryEcpression = "select table_name as tableName,field,field_name as fieldName,remark,expression,express_format as expressionFormat from " + databaseNameConfig.getDmdb() + ".t_bond_indicator_expression where table_name = 'FIN_RATIO_INSU'";
	...
	public List<OriginalData> getData(Long compId, String finDate) {
		String sqlQuery = "select Comp_ID as comp_id,s.*  from " + databaseNameConfig.getDmdb() + "." + TableNameConstant.FIN_SHEET_INSU + " s ";
		if(compId != null){
			sqlQuery += " where comp_id = " + compId;
		}
//		if(finDate != null){//计算公式会涉及到pp py
