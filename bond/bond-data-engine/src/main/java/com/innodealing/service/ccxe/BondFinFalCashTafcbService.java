...
import com.innodealing.dao.jpa.dm.bond.ccxe.BondFinFalCashTafcbRep;
import com.innodealing.dao.jpa.dm.bond.ccxesnapshot.BondFinFalCashTafcbSnapShotRep;
...
@Component
public class BondFinFalCashTafcbService {

    private static final Logger LOG = LoggerFactory.getLogger(BondFinFalCashTafcbService.class);
    ...
    @Autowired
    protected BondFinFalCashTafcbRep bondFinFalCashTafcbRep;
    
    @Autowired
    protected BondFinFalCashTafcbSnapShotRep snapshotBean_BondFinFalCashTafcbRep;
    ...
    public synchronized void pollChange(String beanName, Date syncSideTime) throws Exception {
    	String sqlMaxCcxeid = "select max(ccxeid) from d_bond_fin_fal_cash_tafcb" ;
    	Date primarySideTime = ccxeJdbcTemplate.queryForObject(sqlMaxCcxeid, Date.class);
        LOG.info("max CCXEID of ccxe.d_bond_fin_fal_cash_tafcb is: " + primarySideTime);
    	
        Date syncSideScanHeadTime = (syncSideTime == null) ? ccxeSnapshotJdbcTemplate.queryForObject(sqlMaxCcxeid, Date.class) : syncSideTime;
        LOG.info("max CCXEID of ccxeSnapshot.d_bond_fin_fal_cash_tafcb is: " + syncSideScanHeadTime);
    ...
    private void processTableDataChange(String beanName, Date fromTime, Date toTime) {
        ...
        Long totalCount = bondFinFalCashTafcbRep.getNewListCount(fromTimeStr, toTimeStr);
    ...
    private void batchProcess(String beanName, String fromTime, String toTime, Integer from, Integer count) {
        ...
        List<BondFinFalCashTafcb> rows = bondFinFalCashTafcbRep.getNewList(fromTime, toTime, from, count);
        ...
        {
        	BondFinFalCashTafcb row = rows.get(i);
            String endDate = DateUtils.convertDateToString(row.getEndDate());
            Long snapshotCount = snapshotBean_BondFinFalCashTafcbRep.getCount(row.getId(), row.getBondUniCode(), row.getComUniCode(), endDate);
            if (snapshotCount == 0) {
                eventChannel.sendCreateEvents(beanName, row);
                snapshotBean_BondFinFalCashTafcbRep.save(row);
