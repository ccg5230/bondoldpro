...
import com.innodealing.dao.jpa.dm.bond.ccxe.BondFinGenCashTaccbRep;
import com.innodealing.dao.jpa.dm.bond.ccxesnapshot.BondFinGenCashTaccbSnapShotRep;
...
@Component
public class BondFinGenCashTaccbService {

    private static final Logger LOG = LoggerFactory.getLogger(BondFinGenCashTaccbService.class);
    ...
    @Autowired
    protected BondFinGenCashTaccbRep bondFinGenCashTaccbRep;
    
    @Autowired
    protected BondFinGenCashTaccbSnapShotRep snapshotBean_BondFinGenCashTaccbRep;
    ...
    public synchronized void pollChange(String beanName, Date syncSideTime) throws Exception {
    	String sqlMaxCcxeid = "select max(ccxeid) from d_bond_fin_gen_cash_taccb" ;
    	Date primarySideTime = ccxeJdbcTemplate.queryForObject(sqlMaxCcxeid, Date.class);
        LOG.info("max CCXEID of ccxe.d_bond_fin_gen_cash_taccb is: " + primarySideTime);
    	
        Date syncSideScanHeadTime = (syncSideTime == null) ? ccxeSnapshotJdbcTemplate.queryForObject(sqlMaxCcxeid, Date.class) : syncSideTime;
        LOG.info("max CCXEID of ccxeSnapshot.d_bond_fin_gen_cash_taccb is: " + syncSideScanHeadTime);
    ...
    private void processTableDataChange(String beanName, Date fromTime, Date toTime) {
        ...
        Long totalCount = bondFinGenCashTaccbRep.getNewListCount(fromTimeStr, toTimeStr);
    ...
    private void batchProcess(String beanName, String fromTime, String toTime, Integer from, Integer count) {
        ...
        List<BondFinGenCashTaccb> rows = bondFinGenCashTaccbRep.getNewList(fromTime, toTime, from, count);
        ...
        {
        	BondFinGenCashTaccb row = rows.get(i);
            String endDate = DateUtils.convertDateToString(row.getEndDate());
            Long snapshotCount = snapshotBean_BondFinGenCashTaccbRep.getCount(row.getId(), row.getBondUniCode(), row.getComUniCode(), endDate);
            if (snapshotCount == 0) {
                eventChannel.sendCreateEvents(beanName, row);
                snapshotBean_BondFinGenCashTaccbRep.save(row);
