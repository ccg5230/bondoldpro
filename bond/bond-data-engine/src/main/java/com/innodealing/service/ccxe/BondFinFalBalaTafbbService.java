...
import com.innodealing.dao.jpa.dm.bond.ccxe.BondFinFalBalaTafbbRep;
import com.innodealing.dao.jpa.dm.bond.ccxesnapshot.BondFinFalBalaTafbbSnapShotRep;
...
@Component
public class BondFinFalBalaTafbbService {

    private static final Logger LOG = LoggerFactory.getLogger(BondFinFalBalaTafbbService.class);
    ...
    @Autowired
    protected BondFinFalBalaTafbbRep bondFinFalBalaTafbbRep;
    
    @Autowired
    protected BondFinFalBalaTafbbSnapShotRep snapshotBean_BondFinFalBalaTafbbRep;
    ...
    public synchronized void pollChange(String beanName, Date syncSideTime) throws Exception {
    	String sqlMaxCcxeid = "select max(ccxeid) from d_bond_fin_fal_bala_tafbb" ;
    	Date primarySideTime = ccxeJdbcTemplate.queryForObject(sqlMaxCcxeid, Date.class);
        LOG.info("max CCXEID of ccxe.d_bond_fin_fal_bala_tafbb is: " + primarySideTime);
    	
        Date syncSideScanHeadTime = (syncSideTime == null) ? ccxeSnapshotJdbcTemplate.queryForObject(sqlMaxCcxeid, Date.class) : syncSideTime;
        LOG.info("max CCXEID of ccxeSnapshot.d_bond_fin_fal_bala_tafbb is: " + syncSideScanHeadTime);
    ...
    private void processTableDataChange(String beanName, Date fromTime, Date toTime) {
        ...
        Long totalCount = bondFinFalBalaTafbbRep.getNewListCount(fromTimeStr, toTimeStr);
    ...
    private void batchProcess(String beanName, String fromTime, String toTime, Integer from, Integer count) {
        ...
        List<BondFinFalBalaTafbb> rows = bondFinFalBalaTafbbRep.getNewList(fromTime, toTime, from, count);
        ...
        {
        	BondFinFalBalaTafbb row = rows.get(i);
            String endDate = DateUtils.convertDateToString(row.getEndDate());
            Long snapshotCount = snapshotBean_BondFinFalBalaTafbbRep.getCount(row.getId(), row.getBondUniCode(), row.getComUniCode(), endDate);
            if (snapshotCount == 0) {
                eventChannel.sendCreateEvents(beanName, row);
                snapshotBean_BondFinFalBalaTafbbRep.save(row);
