...
import com.innodealing.dao.jpa.dm.bond.ccxe.BondFinFalProfTafpbRep;
import com.innodealing.dao.jpa.dm.bond.ccxesnapshot.BondFinFalProfTafpbSnapShotRep;
...
@Component
public class BondFinFalProfTafpbService {

    private static final Logger LOG = LoggerFactory.getLogger(BondFinFalProfTafpbService.class);
    ...
    @Autowired
    protected BondFinFalProfTafpbRep bondFinFalProfTafpbRep;
    
    @Autowired
    protected BondFinFalProfTafpbSnapShotRep snapshotBean_BondFinFalProfTafpbRep;
    ...
    public synchronized void pollChange(String beanName, Date syncSideTime) throws Exception {
    	String sqlMaxCcxeid = "select max(ccxeid) from d_bond_fin_fal_prof_tafpb" ;
    	Date primarySideTime = ccxeJdbcTemplate.queryForObject(sqlMaxCcxeid, Date.class);
        LOG.info("max CCXEID of ccxe.d_bond_fin_fal_prof_tafpb is: " + primarySideTime);
    	
        Date syncSideScanHeadTime = (syncSideTime == null) ? ccxeSnapshotJdbcTemplate.queryForObject(sqlMaxCcxeid, Date.class) : syncSideTime;
        LOG.info("max CCXEID of ccxeSnapshot.d_bond_fin_fal_prof_tafpb is: " + syncSideScanHeadTime);
    ...
    private void processTableDataChange(String beanName, Date fromTime, Date toTime) {
        ...
        Long totalCount = bondFinFalProfTafpbRep.getNewListCount(fromTimeStr, toTimeStr);
    ...
    private void batchProcess(String beanName, String fromTime, String toTime, Integer from, Integer count) {
        ...
        List<BondFinFalProfTafpb> rows = bondFinFalProfTafpbRep.getNewList(fromTime, toTime, from, count);
        ...
        {
        	BondFinFalProfTafpb row = rows.get(i);
            String endDate = DateUtils.convertDateToString(row.getEndDate());
            Long snapshotCount = snapshotBean_BondFinFalProfTafpbRep.getCount(row.getId(), row.getBondUniCode(), row.getComUniCode(), endDate);
            if (snapshotCount == 0) {
                eventChannel.sendCreateEvents(beanName, row);
                snapshotBean_BondFinFalProfTafpbRep.save(row);
