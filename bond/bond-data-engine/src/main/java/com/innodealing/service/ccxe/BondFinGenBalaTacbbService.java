...
import com.innodealing.dao.jpa.dm.bond.ccxe.BondFinGenBalaTacbbRep;
import com.innodealing.dao.jpa.dm.bond.ccxesnapshot.BondFinGenBalaTacbbSnapShotRep;
...
@Component
public class BondFinGenBalaTacbbService {

    private static final Logger LOG = LoggerFactory.getLogger(BondFinGenBalaTacbbService.class);
    ...
    @Autowired
    protected BondFinGenBalaTacbbRep bondFinGenBalaTacbbRep;
    
    @Autowired
    protected BondFinGenBalaTacbbSnapShotRep snapshotBean_BondFinGenBalaTacbbRep;
    ...
    public synchronized void pollChange(String beanName, Date syncSideTime) throws Exception {
    	String sqlMaxCcxeid = "select max(ccxeid) from d_bond_fin_gen_bala_tacbb" ;
    	Date primarySideTime = ccxeJdbcTemplate.queryForObject(sqlMaxCcxeid, Date.class);
        LOG.info("max CCXEID of ccxe.d_bond_fin_gen_bala_tacbb is: " + primarySideTime);
    	
        Date syncSideScanHeadTime = (syncSideTime == null) ? ccxeSnapshotJdbcTemplate.queryForObject(sqlMaxCcxeid, Date.class) : syncSideTime;
        LOG.info("max CCXEID of ccxeSnapshot.d_bond_fin_gen_bala_tacbb is: " + syncSideScanHeadTime);
    ...
    private void processTableDataChange(String beanName, Date fromTime, Date toTime) {
        ...
        Long totalCount = bondFinGenBalaTacbbRep.getNewListCount(fromTimeStr, toTimeStr);
    ...
    private void batchProcess(String beanName, String fromTime, String toTime, Integer from, Integer count) {
        ...
        List<BondFinGenBalaTacbb> rows = bondFinGenBalaTacbbRep.getNewList(fromTime, toTime, from, count);
        ...
        {
        	BondFinGenBalaTacbb row = rows.get(i);
            String endDate = DateUtils.convertDateToString(row.getEndDate());
            Long snapshotCount = snapshotBean_BondFinGenBalaTacbbRep.getCount(row.getId(), row.getBondUniCode(), row.getComUniCode(), endDate);
            if (snapshotCount == 0) {
                eventChannel.sendCreateEvents(beanName, row);
                snapshotBean_BondFinGenBalaTacbbRep.save(row);
